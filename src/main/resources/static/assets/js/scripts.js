        ...
        startIndexing:{
            ...
            action: function(result, $this){
                if (result.result){
                    ...
                    if(result.error === 'Индексация уже запущена' && $this.is('[data-btntype="check"]')){
                        shiftCheck($this);
                    }
                }
            }
                ...
                if (result.result){
                    ...
                    result.data.forEach(function(page){
                        $content.append('<div class="SearchResult-block">' +
                            '<a href="' + page.site + page.url +'" target="_blank" class="SearchResult-siteTitle">' +
                    ...
                    if ($this.next('.API-error').length) {
                        $this.next('.API-error').text(result.error);
                         ...
                         var $searchResults = $('.SearchResult'),
                          $content = $searchResults.find('.SearchResult-content').empty();

                         $searchResults.find('.SearchResult-amount').text(0);
